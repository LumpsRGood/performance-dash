...
def read_excel_with_header_search(
    ...
    file.seek(0)
    return pd.read_excel(file, sheet_name=sheet_name, header=default_header)
...
def uploaded_workbook_date(file):
    file.seek(0)
    raw = pd.read_excel(file, sheet_name=0, header=None, nrows=3)
    file.seek(0)
    text = " ".join(str(v) for v in raw.fillna("").to_numpy().ravel())
    match = re.search(r"(\d{1,2}/\d{1,2}/\d{4})", text)
    if not match:
        match = re.search(r"(\d{2}-\d{2}-\d{4})", file.name)
        if match:
            return pd.to_datetime(match.group(1), format="%m-%d-%Y").date()
        raise ValueError(f"Could not determine business date from {file.name}")
    return pd.to_datetime(match.group(1)).date()


def insert_manual_import_run(cur, business_date, report_type, file):
    digest = hashlib.sha256(uploaded_file_bytes(file)).hexdigest()
    cur.execute(
        """
...
def upsert_manual_rosnet_rows(cur, rows, run_id, report_type):
    for row in rows:
        cur.execute(
            """
            ...
            do update set
                store_label = coalesce(excluded.store_label, public.foh_daily_metrics.store_label),
            ...
            """,
            (
                row["business_date"], row["store_number"], row["store_label"], row["employee_name"],
                row["support_staff"], row.get("dine_in_bev_pct"), row.get("bev_weight"),
                row.get("ppa"), row.get("ppa_weight"), row.get("net_sales"),
                run_id if report_type == "beverage" else None,
                run_id if report_type == "ppa" else None,
            ),
        )


def save_manual_rosnet_uploads(beverage_files, ppa_files):
    cfg = get_db_config()
    if not cfg:
        raise RuntimeError("Database credentials are not configured in Streamlit secrets.")
    conn = psycopg2.connect(host=cfg["DB_HOST"], port=cfg["DB_PORT"], dbname=cfg["DB_NAME"], user=cfg["DB_USER"], password=cfg["DB_PASSWORD"], sslmode="require")
    summary = []
    try:
        cur = conn.cursor()
        for report_type, files in (("beverage", beverage_files or []), ("ppa", ppa_files or [])):
            for file in files:
                business_date = uploaded_workbook_date(file)
                df = process_beverage_file(file) if report_type == "beverage" else process_ppa_file(file)
                rows = []
                for _, r in df.iterrows():
                    store = normalize_store_number(r["Store"])
                    ...
                    row = {
                        ...
                        "store_label": get_store_label(store),
                        "employee_name": str(r["Server"]).strip(),
                        "support_staff": is_support_staff(r["Server"]),
                    }
                    if report_type == "beverage":
                        row.update({"dine_in_bev_pct": None if pd.isna(r["Dine In Bev %"]) else float(r["Dine In Bev %"]), "bev_weight": None if pd.isna(r["Bev Weight"]) else float(r["Bev Weight"])})
                    else:
                        row.update({"ppa": None if pd.isna(r["PPA"]) else float(r["PPA"]), "ppa_weight": None if pd.isna(r["PPA Weight"]) else float(r["PPA Weight"]), "net_sales": None if pd.isna(r["Net Sales"]) else float(r["Net Sales"])})
                    rows.append(row)
                run_id = insert_manual_import_run(cur, business_date, "employee_contest_detail" if report_type == "beverage" else "employee_sales_statistics", file)
                upsert_manual_rosnet_rows(cur, rows, run_id, report_type)
                summary.append((business_date, report_type, len(rows), file.name))
        conn.commit()
        return summary
    except Exception:
        conn.rollback()
...
if data_source == "FOH Database":
    ...
    if beverage_files or ppa_files:
        st.markdown("### Save Rosnet uploads to Supabase")
        st.caption("Each daily Beverage or PPA workbook is saved to its own business date. Existing Tray/tablet/turn-time fields are preserved.")
        if st.button("Save Rosnet files to database", type="primary", use_container_width=True):
            try:
                with st.spinner("Saving Rosnet files to Supabase..."):
                    saved = save_manual_rosnet_uploads(beverage_files, ppa_files)
                st.success(f"Saved {len(saved)} Rosnet file(s) to Supabase.")
                st.dataframe(pd.DataFrame(saved, columns=["Business Date", "Type", "Rows", "File"]), hide_index=True, use_container_width=True)
                st.cache_data.clear()
            except Exception as exc:
